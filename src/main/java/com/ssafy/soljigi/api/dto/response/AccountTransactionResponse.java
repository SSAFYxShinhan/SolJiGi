...
public class AccountTransactionResponse {
	...
	public static AccountTransactionResponse of(Account account, LocalDateTime dateTime) {
		...
		dataBody.set거래내역(transactions.stream()
			.filter((transaction) -> (
				Period.between(transaction.getTransactionDateTime().toLocalDate(), dateTime.toLocalDate()).getDays()
					== 0))
			.toList().stream()
			.map(transaction -> {
